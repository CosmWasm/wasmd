...
func InitializeWasmApp(b testing.TB, db dbm.DB, numAccounts int) AppInfo {
	...
	var r *rand.Rand
	storeTx, err := helpers.GenTx(r, txGen, []sdk.Msg{&storeMsg}, nil, 55123123, "", []uint64{0}, []uint64{0}, minter)
	require.NoError(b, err)
	...
	initTx, err := helpers.GenTx(r, txGen, []sdk.Msg{&initMsg}, nil, gasWanted, "", []uint64{0}, []uint64{1}, minter)
	require.NoError(b, err)
