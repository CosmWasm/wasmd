...
type ChainAppFactory func(tb testing.TB, valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, chainID string, opts []wasmkeeper.Option, balances ...banktypes.Balance) ChainApp

// DefaultWasmAppFactory instantiates and sets up the default wasmd app
func DefaultWasmAppFactory(tb testing.TB, valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, chainID string, opts []wasmkeeper.Option, balances ...banktypes.Balance) ChainApp {
	return app.SetupWithGenesisValSet(tb, valSet, genAccs, chainID, opts, balances...)
}

// NewDefaultTestChain initializes a new test chain with a default of 4 validators
// Use this function if the tests do not need custom control over the validator set
func NewDefaultTestChain(tb testing.TB, coord *Coordinator, chainID string, opts ...wasmkeeper.Option) *TestChain {
	return NewTestChain(tb, coord, DefaultWasmAppFactory, chainID, opts...)
}

// NewTestChain initializes a new test chain with a default of 4 validators
// Use this function if the tests do not need custom control over the validator set
func NewTestChain(tb testing.TB, coord *Coordinator, appFactory ChainAppFactory, chainID string, opts ...wasmkeeper.Option) *TestChain {
	...
	return NewTestChainWithValSet(tb, coord, appFactory, chainID, valSet, signersByAddress, opts...)
}
...
func NewTestChainWithValSet(tb testing.TB, coord *Coordinator, appFactory ChainAppFactory, chainID string, valSet *cmttypes.ValidatorSet, signers map[string]cmttypes.PrivValidator, opts ...wasmkeeper.Option) *TestChain {
	...
	wasmApp := appFactory(tb, valSet, genAccs, chainID, opts, genBals...)
...
func (chain *TestChain) commitBlock(res *abci.FinalizeBlockResponse) {
	...
	chain.NextVals = ibctesting.ApplyValSetChanges(chain.TB, chain.Vals, res.ValidatorUpdates)
...
func (chain *TestChain) sendWithSigner(
	...
	require.Len(chain.TB, blockResp.TxResults, 1)
...
var unusedHash = tmhash.Sum([]byte{0x00})
...
func (chain *TestChain) GetTimeoutTimestamp() uint64 {
	return uint64(chain.GetContext().BlockTime().UnixNano()) + ibctesting.DefaultTimeoutTimestampDelta
}
