...
func TestEurekaReceiveEntrypoint(t *testing.T) {
	...
	chainB := wasmibctesting.NewWasmTestChain(coord.GetChain(ibctesting.GetChainID(1)))
	...
	mockContractAddrB := chainB.SeedNewContractInstance()
	contractPortB := wasmkeeper.PortIDForContract(mockContractAddrB)
	...
	_, err := path.EndpointB.MsgSendPacket(timeoutTimestamp, mockv2.NewMockPayload(contractPortB, contractPortA))
