...
func TestIBC2SendMsg(t *testing.T) {
	...
	payload := channeltypesv2.NewPayload(contractPortB, contractPortA, "V2", "json", packetMsgBz)

	// Message timeout
	timeoutTimestamp := uint64(chainB.GetContext().BlockTime().Add(time.Minute).Unix())

	msgSendPacket := channeltypesv2.NewMsgSendPacket(path.EndpointB.ClientID, timeoutTimestamp, chainB.SenderAccount.GetAddress().String(), payload)
	fmt.Println("msgSendPacket: " + msgSendPacket.String())

	// send packet
	fmt.Println("\n\nSourceClient: " + msgSendPacket.SourceClient)

	// _, err = chainB.App.GetIBCKeeper().ChannelKeeperV2.SendPacket(chainB.GetContext(), msgSendPacket)
	// _, err = chainB.SendMsgs(msgSendPacket)
	packet, err := path.EndpointB.MsgSendPacket(timeoutTimestamp, payload)
