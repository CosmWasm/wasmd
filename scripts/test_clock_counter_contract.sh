...
sleep 1
store_info=$(oraid query tx $store_txhash --output json)
code_id=$(echo $store_info | jq -r '.events[4].attributes[] | select(.key | contains("code_id")).value')
...
oraid tx clock add-contract $contract_address $CONTRACT_GAS_LIMIT "$TITLE" "$INITIAL_DEPOSIT" "$DESCRIPTION" $VALIDATOR1_ARGS $ARGS --output json
# add_contract_result=$(oraid tx clock add-contract $contract_address $CONTRACT_GAS_LIMIT "$TITLE" "$INITIAL_DEPOSIT" "$DESCRIPTION" $VALIDATOR1_ARGS $ARGS --output json)
