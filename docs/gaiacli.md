...
`wasmcli` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not. Let us set it up properly. In order to install it, follow the [installation procedure](./installation.md).

### Setting up wasmcli 

The main command used to set up `wasmcli` is the following:
...
- `cosmos`
  - Derived from account keys generated by `wasmcli keys add`
...
- `cosmosvalconspub`
  - Generated when the node is created with `wasmd init`.
  - Get this value with `wasmd tendermint show-validator`
...
Older versions of `wasmcli` used store keys in the user's home directory. If you are migrating
from an old version of `wasmcli` you will need to migrate your old keys into your operating system's
credentials storage by running the following command:

```bash
wasmcli keys migrate
```
...
When broadcasting transactions, `wasmcli` accepts a `--broadcast-mode` flag. This
...
```bash
wasmcli tx send <sender_key_name_or_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id>
```
...
```bash
wasmcli tx send <sender_key_name_or_address> <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --dry-run
```
...
```bash
wasmcli tx send <sender_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id> \
...
::: tip Note
The `--generate-only` flag prevents `wasmcli` from accessing the local keybase.
...
```bash
wasmcli query minting params
```

To query for the current inflation value:

```bash
wasmcli query minting inflation
```

To query for the current annual provisions value:

```bash
wasmcli query minting annual-provisions
```
...
where `[name]` is the name of the key you specified when you initialized `wasmd`.
...
```bash
wasmcli query distribution community-pool
```
...
```bash
wasmcli tx send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 1000000uatom \
