...
// AnteHandler is a function signature for ante handlers.
...
// WithBlockHeight returns a copy of the context with the given block height.
...
// dummyKVStore is an in-memory KV store.
...
// Define the key and context helpers for the TX counter.
...
// It passes the updated counter via the context.
...
// -----------------------------------------------------------------------------
// Test Functions (simulated testing)
// -----------------------------------------------------------------------------
...
func testSameBlock() error {
	...
	// A dummy next handler that returns the context unmodified.
...
func testNewBlock() error {
	...
	// Simulate a new block by updating the context.
	ctx = NewContext(101)
	ctx, err = decorator.AnteHandle(ctx, nil, false, nextHandler)
	if err != nil {
	...
	fmt.Printf("testNewBlock passed: block 100 counter=%d, block 101 counter=%d\n", counter, newCounter)
	return nil
}
