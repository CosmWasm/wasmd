...
func TestContractInfoSetExtension(t *testing.T) {
	...
	aNestedProtobufExt := func() ContractInfoExtension {
		...
		myExtension, err := govtypes.NewProposal(&govtypes.TextProposal{Title: "bar"}, 1, anyTime, anyTime, true)
		require.NoError(t, err)
...
func TestContractInfoReadExtension(t *testing.T) {
	anyTime := time.Now().UTC()
	myExtension, err := govtypes.NewProposal(&govtypes.TextProposal{Title: "foo"}, 1, anyTime, anyTime, true)
	require.NoError(t, err)
