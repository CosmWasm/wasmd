...
func (k Keeper) OnOpenChannel(
	...
	res, gasReport, execErr := k.wasmVM.IBCChannelOpen(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization)
...
func (k Keeper) OnConnectChannel(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasReport, execErr := k.wasmVM.IBCChannelConnect(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization)
	k.consumeRuntimeGas(ctx, gasReport.UsedInternally)
	if execErr != nil {
		return errorsmod.Wrap(types.ErrExecuteFailed, execErr.Error())
	}
...
func (k Keeper) OnCloseChannel(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasReport, execErr := k.wasmVM.IBCChannelClose(codeInfo.CodeHash, params, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization)
	k.consumeRuntimeGas(ctx, gasReport.UsedInternally)
	if execErr != nil {
		return errorsmod.Wrap(types.ErrExecuteFailed, execErr.Error())
	}
...
func (k Keeper) OnRecvPacket(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasReport, execErr := k.wasmVM.IBCPacketReceive(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization)
	k.consumeRuntimeGas(ctx, gasReport.UsedInternally)
	if execErr != nil {
...
func (k Keeper) OnAckPacket(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasReport, execErr := k.wasmVM.IBCPacketAck(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization)
	k.consumeRuntimeGas(ctx, gasReport.UsedInternally)
	if execErr != nil {
		return errorsmod.Wrap(types.ErrExecuteFailed, execErr.Error())
	}
...
func (k Keeper) OnTimeoutPacket(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasReport, execErr := k.wasmVM.IBCPacketTimeout(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization)
	k.consumeRuntimeGas(ctx, gasReport.UsedInternally)
	if execErr != nil {
		return errorsmod.Wrap(types.ErrExecuteFailed, execErr.Error())
	}
