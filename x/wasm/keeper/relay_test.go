...
func TestOnOpenChannel(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			m.IBCChannelOpenFn = func(codeID wasmvm.Checksum, env wasmvmtypes.Env, msg wasmvmtypes.IBCChannelOpenMsg, store wasmvm.KVStore, goapi wasmvm.GoAPI, querier wasmvm.Querier, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBC3ChannelOpenResponse, wasmvmtypes.GasReport, error) {
				assert.Equal(t, myMsg, msg)
				return &wasmvmtypes.IBC3ChannelOpenResponse{}, wasmvmtypes.NewGasReport(gasLimit, spec.contractGas*DefaultGasMultiplier), spec.contractErr
			}
...
func TestOnConnectChannel(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			m.IBCChannelConnectFn = func(codeID wasmvm.Checksum, env wasmvmtypes.Env, msg wasmvmtypes.IBCChannelConnectMsg, store wasmvm.KVStore, goapi wasmvm.GoAPI, querier wasmvm.Querier, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCBasicResponse, wasmvmtypes.GasReport, error) {
				assert.Equal(t, msg, myMsg)
				return spec.contractResp, wasmvmtypes.NewGasReport(gasLimit, myContractGas*DefaultGasMultiplier), spec.contractErr
			}
...
func TestOnCloseChannel(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			m.IBCChannelCloseFn = func(codeID wasmvm.Checksum, env wasmvmtypes.Env, msg wasmvmtypes.IBCChannelCloseMsg, store wasmvm.KVStore, goapi wasmvm.GoAPI, querier wasmvm.Querier, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCBasicResponse, wasmvmtypes.GasReport, error) {
				assert.Equal(t, msg, myMsg)
				return spec.contractResp, wasmvmtypes.NewGasReport(gasLimit, myContractGas*DefaultGasMultiplier), spec.contractErr
			}
...
func TestOnRecvPacket(t *testing.T) {
	...
	specs := map[string]struct {
		...
		contractErr        error
		overwriteMessenger *wasmtesting.MockMessageHandler
		mockReplyFn        func(codeID wasmvm.Checksum, env wasmvmtypes.Env, reply wasmvmtypes.Reply, store wasmvm.KVStore, goapi wasmvm.GoAPI, querier wasmvm.Querier, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.Response, wasmvmtypes.GasReport, error)
		expContractGas     sdk.Gas
		expAck             []byte
		expErr             bool
		expPanic           bool
		...
		"submessage reply can overwrite ack data": {
			...
			mockReplyFn: func(codeID wasmvm.Checksum, env wasmvmtypes.Env, reply wasmvmtypes.Reply, store wasmvm.KVStore, goapi wasmvm.GoAPI, querier wasmvm.Querier, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.Response, wasmvmtypes.GasReport, error) {
				return &wasmvmtypes.Response{Data: []byte("myBetterAck")}, wasmvmtypes.EmptyGasReport(gasLimit), nil
			},
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			m.IBCPacketReceiveFn = func(codeID wasmvm.Checksum, env wasmvmtypes.Env, msg wasmvmtypes.IBCPacketReceiveMsg, store wasmvm.KVStore, goapi wasmvm.GoAPI, querier wasmvm.Querier, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCReceiveResult, wasmvmtypes.GasReport, error) {
...
func TestOnAckPacket(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			m.IBCPacketAckFn = func(codeID wasmvm.Checksum, env wasmvmtypes.Env, msg wasmvmtypes.IBCPacketAckMsg, store wasmvm.KVStore, goapi wasmvm.GoAPI, querier wasmvm.Querier, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCBasicResponse, wasmvmtypes.GasReport, error) {
...
func TestOnTimeoutPacket(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			m.IBCPacketTimeoutFn = func(codeID wasmvm.Checksum, env wasmvmtypes.Env, msg wasmvmtypes.IBCPacketTimeoutMsg, store wasmvm.KVStore, goapi wasmvm.GoAPI, querier wasmvm.Querier, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCBasicResponse, wasmvmtypes.GasReport, error) {
				assert.Equal(t, myPacket, msg.Packet)
				return spec.contractResp, wasmvmtypes.NewGasReport(gasLimit, myContractGas*DefaultGasMultiplier), spec.contractErr
			}
