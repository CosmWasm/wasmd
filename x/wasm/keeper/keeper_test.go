...
func TestCreateStoresInstantiatePermission(t *testing.T) {
	var (
		deposit                = sdk.NewCoins(sdk.NewInt64Coin("denom", 100000))
		myAddr  sdk.AccAddress = bytes.Repeat([]byte{1}, 32)
	)
...
func TestInstantiateWithDeposit(t *testing.T) {
	var (
		bob  = bytes.Repeat([]byte{1}, 32)
		fred = bytes.Repeat([]byte{2}, 32)
...
func TestInstantiateWithPermissions(t *testing.T) {
	var (
		deposit   = sdk.NewCoins(sdk.NewInt64Coin("denom", 100000))
		myAddr    = bytes.Repeat([]byte{1}, 32)
		otherAddr = bytes.Repeat([]byte{2}, 32)
		anyAddr   = bytes.Repeat([]byte{3}, 32)
	)
