...
func TestMultipleReplies(t *testing.T) {
	...
	// Assert that from the collection of submessages the `data` field will be set by the
	// last submessage.
	execMsg, err = json.Marshal(repliesMsgTemplate)
	require.NoError(t, err)
	em = sdk.NewEventManager()
	...
	// Assert that from the collection of submessages the `data` field will be
	...
	// Assert that in scenario C1 -> C4 -> C5 if C4 and C2 doesn't set `data`,
	...
	// Assert that catching the error prevents the rollback of the whole transaction
	// TODO: this testcase probably indicates a bug in wasmd
