...
func TestCopyDestroyUnmanagedVector(t *testing.T) {
	{
		...
		invalidPtr := []byte{0x42}
		uv := constructUnmanagedVector(cbool(true), cu8_ptr(unsafe.Pointer(&invalidPtr[0])), cusize(0xBB), cusize(0xAA))
		copy := copyAndDestroyUnmanagedVector(uv)
		require.Nil(t, copy)
	}
	{
		// Capacity is 0, so no allocation happened. Do not access the pointer.
		uv := constructUnmanagedVector(cbool(false), cu8_ptr(nil), cusize(0), cusize(0))
		copy := copyAndDestroyUnmanagedVector(uv)
