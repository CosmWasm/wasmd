...
func (coord *Coordinator) CreateTransferChannels(
	...
	return coord.CreateChannel(chainA, chainB, connA, connB, "transfer", "transfer", order)
}
...
func (coord *Coordinator) CreateChannel(
	chainA, chainB *TestChain,
	connA, connB *TestConnection,
	sourcePortID, counterpartPortID string,
	order channeltypes.Order,
) (TestChannel, TestChannel) {

	channelA, channelB, err := coord.ChanOpenInit(chainA, chainB, connA, connB, sourcePortID, counterpartPortID, order)
	require.NoError(coord.t, err)
...
func (coord *Coordinator) ChanOpenInit(
	...
	counterpartyChannel := counterpartyConnection.AddTestChannel(counterpartPortID)
