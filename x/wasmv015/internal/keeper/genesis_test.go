...
import (
	...
	wasmTypes "github.com/CosmWasm/wasmd/x/wasmv015/internal/types"
...
func TestSupportedGenMsgTypes(t *testing.T) {
	...
	require.NoError(t, err)
	var (
		myAddress          sdk.AccAddress = bytes.Repeat([]byte{1}, v040auth.AddrLen)
		verifierAddress    sdk.AccAddress = bytes.Repeat([]byte{2}, v040auth.AddrLen)
		beneficiaryAddress sdk.AccAddress = bytes.Repeat([]byte{3}, v040auth.AddrLen)
	)
	const denom = "stake"
